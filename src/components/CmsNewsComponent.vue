...
export default {
    ...
    data() {
        return {
            localArticles: [], // All articles retrieved from the API
    ...
    computed: {
        totalPages() {
            return Math.ceil(this.localArticles.length / this.articlesPerPage);
        },
        paginatedArticles() {
            const startIndex = (this.currentPage - 1) * this.articlesPerPage;
            const endIndex = startIndex + this.articlesPerPage;
            return this.localArticles.slice(startIndex, endIndex);
        },
    },
    methods: {
        prevPage() {
    ...
    mounted() {
        this.$axios.get('/api/news')
            .then(response => {
                this.localArticles = response.data;
            })
            .catch(error => {
                console.error(error);
            });
    ...
    watch: {
        articles(newValue) {
            this.localArticles = [...newValue]; // Create a copy of the prop
        }
    },
}
</script>
